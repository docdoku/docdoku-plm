...
@NamedQueries({
        ...
        @NamedQuery(name="ProductInstanceMaster.findByPart", query="SELECT DISTINCT pim FROM ProductInstanceMaster pim JOIN ProductBaseline pb JOIN BaselinedPart bp WHERE pim.instanceOf.id = pb.configurationItem.id AND pb.partCollection.id = bp.partCollection.id AND bp.targetPart.partRevision.partMasterNumber = :partNumber AND bp.targetPart.partRevision.partMasterWorkspaceId = :workspaceId AND bp.targetPartVersion = :partVersion")
})
