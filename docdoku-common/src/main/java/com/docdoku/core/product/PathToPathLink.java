...
@NamedQueries({
        ...
        @NamedQuery(name="PathToPathLink.findPathToPathLinkBySourceAndTarget", query="SELECT DISTINCT p FROM PathToPathLink p JOIN ProductInstanceIteration pi WHERE p member of pi.pathToPathLinks AND pi = :productInstanceIteration AND (p.sourcePath = :source AND p.targetPath = :target OR p.sourcePath = :target AND p.targetPath = :source)"),
