...
@NamedQueries({
        ...
        @NamedQuery(name="PartRevision.countByWorkspace.filterACLEntry", query="SELECT count(pr) FROM PartRevision pr WHERE pr.partMaster.workspace.id = :workspaceId and (pr.acl is null or exists(SELECT au from ACLUserEntry au WHERE au.principal = :user AND au.permission <> com.docdoku.core.security.ACLPermission.FORBIDDEN AND au.acl = pr.acl) or exists(SELECT aug from ACLUserGroupEntry aug WHERE aug.principal in (SELECT ug from UserGroup ug WHERE :user member of ug.users ) AND aug.permission <> com.docdoku.core.security.ACLPermission.FORBIDDEN AND aug.acl = pr.acl))"),
