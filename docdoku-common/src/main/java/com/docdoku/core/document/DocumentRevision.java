...
@NamedQueries({
        ...
        @NamedQuery(name = "DocumentRevision.findByWorkspace.filterACLEntry", query = "SELECT dr FROM DocumentRevision dr WHERE dr.documentMasterWorkspaceId = :workspaceId and (dr.acl is null or exists(SELECT au from ACLUserEntry au WHERE au.principal = :user AND au.permission <> com.docdoku.core.security.ACLPermission.FORBIDDEN AND au.acl = dr.acl) or exists(SELECT aug from ACLUserGroupEntry aug WHERE aug.principal in (SELECT ug from UserGroup ug WHERE :user member of ug.users ) AND aug.permission <> com.docdoku.core.security.ACLPermission.FORBIDDEN AND aug.acl = dr.acl)) AND dr.location.completePath NOT LIKE :excludedFolders ORDER BY dr.documentMasterId ASC"),
        @NamedQuery(name = "DocumentRevision.countByWorkspace.filterACLEntry", query = "SELECT count(dr) FROM DocumentRevision dr WHERE dr.documentMasterWorkspaceId = :workspaceId and (dr.acl is null or exists(SELECT au from ACLUserEntry au WHERE au.principal = :user AND au.permission <> com.docdoku.core.security.ACLPermission.FORBIDDEN AND au.acl = dr.acl) or exists(SELECT aug from ACLUserGroupEntry aug WHERE aug.principal in (SELECT ug from UserGroup ug WHERE :user member of ug.users ) AND aug.permission <> com.docdoku.core.security.ACLPermission.FORBIDDEN AND aug.acl = dr.acl)) AND dr.location.completePath NOT LIKE :excludedFolders"),
