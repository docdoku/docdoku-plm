...
@NamedQueries ({
        ...
        @NamedQuery(name="DocumentRevision.findWithAssignedTasksForUser", query="SELECT d FROM DocumentRevision d, Task t WHERE t.activity.workflow = d.workflow AND d.workflow IS NOT NULL AND d.documentMasterWorkspaceId = :workspaceId AND :user MEMBER OF t.assignedUsers or :login = ANY (SELECT u.login FROM UserGroup g JOIN g.users u where g.workspace.id = :workspaceId AND g MEMBER OF t.assignedGroups)"),
