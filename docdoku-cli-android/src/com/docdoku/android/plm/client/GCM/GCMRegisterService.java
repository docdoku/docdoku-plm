...
public class GCMRegisterService extends Service implements HttpDeleteTask.HttpDeleteListener, HttpPutTask.HttpPutListener{
    ...
    /**
     * Value of the <code>Intent Extra</code> indicating that the GCM Id should be sent to the server
     */
    public static final int ACTION_SEND_ID = 1;
    /**
     * Value of the <code>Intent Extra</code> indicating that the GCM Id should be removed from the server
     */
     ...
     * Depending on the <code>Extra</code>s contained in the <code>Intent</code>, calls {@link #getGCMId()} to send the
    ...
    /**
     * Searches in <code>SharedPreferences</code> for a GCM Id to send to the server. If no GCM Id is found or if the GCM
    ...
    /**
     * Start an <code>AsyncTask</code> to connect to the GCM server to ask for a new GCM Id. Once it is received, calls
     * {@link #saveGCMId(String) saveGCMId(gcmId)} then {@link #sendGCMId(String) sendGCMId(gcmId)}.
     */
    ...
    /**
     * Saves the GCM Id to the <code>SharedPreferences</code> along with the data about the current app version and the current date.
     *
    ...
    /**
     * Sends the GCM Id to the server in a {@link HttpPutTask}
     * @param gcmId
     */
